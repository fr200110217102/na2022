...
// n 是分辨率，即网格每条边的格点数。
// 输出是解 sol，即所有格点处的函数值。
class Neuman_Normal_Solver : public BVPSolver{
private:
	int n, d;
	double h;				// 网格宽度
	...
	void Matrix_Generator() {
		...
		for (int j = 1; j <= n; ++ j) {
			coef[ID(0,j)][ID(0,j)] = -1.5 / h;
			coef[ID(0,j)][ID(1,j)] = 2 / h;
			coef[ID(0,j)][ID(2,j)] = -0.5 / h;
			rhs[ID(0,j)] = g(0, j*h);
			coef[ID(n+1,j)][ID(n+1,j)] = 1.5 / h;
			coef[ID(n+1,j)][ID(n,j)] = -2 / h;
			coef[ID(n+1,j)][ID(n-1,j)] = 0.5 / h;
			rhs[ID(n+1,j)] = g(1, j*h);
		}
		for (int i = 1; i <= n; ++ i) {
			coef[ID(i,0)][ID(i,0)] = -1.5 / h;
			coef[ID(i,0)][ID(i,1)] = 2 / h;
			coef[ID(i,0)][ID(i,2)] = -0.5 / h;
			rhs[ID(i,0)] = g(i*h, 0);
			coef[ID(i,n+1)][ID(i,n+1)] = 1.5 / h;
			coef[ID(i,n+1)][ID(i,n)] = -2 / h;
			coef[ID(i,n+1)][ID(i,n-1)] = 0.5 / h;
		...
		coef[ID(1,1)][ID(1,1)] += 1;
	}
public:
	Neuman_Normal_Solver(int n, Function_2D<double>& f, Function_2D<double>& g) :
